...
* ・メールアドレスのname属性の値が「Email」ではない場合、以下必須設定箇所の「$メールアドレス」の値も変更下さい。
...
$Referer_check_domain = "http://xxxxxx";
...
$thanksPage = "https://roleup.co.jp/dd-lp";
...
$require = array('お問い合わせ項目','お名前','会社名','メールアドレス','電話番号','個人情報保護方針に同意する');
...
// 送る場合は、フォーム側のメール入力欄のname属性の値が上記「$メールアドレス」で指定した値と同じである必要があります
$remail = 1;

//自動返信メールの送信者欄に表示される名前　※あなたの名前や会社名など（もし自動返信メールの送信者名が文字化けする場合ここは空にしてください）
$refrom_name = "株式会社ROLE UP";
...
//※デフォルトは「する」。特に理由がなければ変更しないで下さい。メール入力欄のname属性の値が上記「$メールアドレス」で指定した値である必要があります。
$mail_check = 1;
...
if(empty($errm)){
	foreach($_POST as $key=>$val) {
		...
		if($key == $メールアドレス) $post_mail = h($val);
		if($key == $メールアドレス && $mail_check == 1 && !empty($val)){
			if(!checkMail($val)){
...
<title>完了画面</title>
<!-- Google tag (gtag.js) 
<script async src="https://www.googletagmanager.com/gtag/js?id=AW-11200238041"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'AW-11200238041');
</script>-->
<!-- Google tag (gtag.js) --> 
 <!-- <script async src="https://www.googletagmanager.com/gtag/js?id=G-CW9GJYECE6"></script> 
 <script> window.dataLayer = window.dataLayer || []; function gtag(){dataLayer.push(arguments);} gtag('js', new Date()); gtag('config', 'G-CW9GJYECE6'); </script> -->

<!-- Event snippet for フォーム送信 conversion page -->
<!-- <script>
  gtag('event', 'conversion', {'send_to': 'AW-11200238041/VHR9COm39qUYENmj2Nwp'});
</script> -->
...
function copyright(){
	echo '<a style="display:block;text-align:center;margin:15px 0;font-size:11px;color:#aaa;text-decoration:none" href="https://roleup.co.jp/" target="_blank">©2023　ROLE UP</a>';
}
//----------------------------------------------------------------------
//  関数定義(END)
//----------------------------------------------------------------------
